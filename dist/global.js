(() => {
  ...
  Webflow.push(() => {
    ...
    if (Webflow.env("editor") === void 0) {
      lenis = new Lenis();
      lenis.on("scroll", ScrollTrigger2.refresh);
    ...
    document.querySelectorAll(".link-gsap").forEach((link) => {
      ...
      const enterTimeline = gsapWithCSS.timeline({ paused: true });
      const leaveTimeline = gsapWithCSS.timeline({ paused: true });
      enterTimeline.to(underline, { width: "100%", duration: 0.4, ease: "power2.out" }).to(text, { y: 2, duration: 0.2, ease: "power2.out" }, "<").to(underlineWrap, { y: -2, duration: 0.2, ease: "power2.out" }, "<");
      leaveTimeline.to(underline, { width: "0%", duration: 0.4, ease: "power2.out" }).to(text, { y: 0, duration: 0.2, ease: "power2.out" }, "<").to(underlineWrap, { y: 0, duration: 0.2, ease: "power2.out" }, "<");
    ...
    if (preloader) {
      let preloaderAnimation = gsapWithCSS.timeline();
      ...
      gsapWithCSS.set(cover, { opacity: 1 });
      ...
      document.querySelectorAll("a").forEach(function(anchor) {
        anchor.addEventListener("click", function(e) {
          if (this.hostname === window.location.host && this.getAttribute("href").indexOf("#") === -1 && this.getAttribute("target") !== "_blank") {
            e.preventDefault();
            const destination = this.getAttribute("href");
            gsapWithCSS.set(preloader, { display: "block" });
    ...
    window.onpageshow = function(event2) {
      if (event2.persisted) {
